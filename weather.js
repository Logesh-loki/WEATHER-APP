...
async function getdata(lat,lon){  
     let url1=`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&appid=32a11364bc771a0e9fb542b8ebcf613e`;
...
async function search_val(){
    ...
    let url=`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&APPID=3a5304c8f8c5c58d54b426fd1d5aa9d9`
    try{
        let res=await fetch(url)
        let data=await res.json()
...
wind.innerHTML=`&nbsp &nbsp wind-speed ${data.current.wind_speed}km/h`
...
if(data.current.weather[0].main=="Haze"){
    ...
    // back.style.backgroundImage = "fixed"
}
if(data.current.weather[0].main=="Rain"){
    back.style.backgroundImage = "url('https://bestanimations.com/media/rain/859858665rain-window-city-gif-21.gif')"
    // back.style.backgroundImage = "fixed"
}
if(data.current.weather[0].main=="Clouds"){
    back.style.backgroundImage = "url('https://i.pinimg.com/originals/aa/96/97/aa9697a3f7a61389675b8dc109518753.gif')"
